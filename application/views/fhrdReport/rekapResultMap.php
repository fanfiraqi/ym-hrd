<? //if ($display==0){
	...
	foreach ($arrCabang as $cabang) {
		...
		$str=" select ".implode(", ",$arrField)." from ".$nmTable." tbl, (select * from pegawai  ".($jenis=="resign"?" where status_aktif=0":"where status_aktif=1").") P 
				where tbl.nik=P.nik and P.id_cabang=".$cabang->ID_CABANG." and ".(sizeof($fieldKey)<=1?"date_format(".$fieldKey[0].",'%Y%m') ='".$thn.$bln."'":"(date_format(".$fieldKey[0].",'%Y%m') ='".$thn.$bln."' or date_format(".$fieldKey[1].",'%Y%m') ='".$thn.$bln."')")." order by P.NAMA";	//blm filter bln-thn

		if ($jenis=="lembur"){
		$str=" select ".implode(", ",$arrField)." from ".$nmTable.($jenis=="lembur"?"":" tbl").", (select * from pegawai  ".($jenis=="resign"?"where status_aktif=0":"where status_aktif=1").") P 
				where ".($jenis=="lembur"?"l.nik=P.nik":"tbl.nik=P.nik")." and l.no_trans=d.no_trans and P.id_cabang=".$cabang->ID_CABANG." and ".(sizeof($fieldKey)<=1?"date_format(".$fieldKey[0].",'%Y%m') ='".$thn.$bln."'":"(date_format(".$fieldKey[0].",'%Y%m') ='".$thn.$bln."' or date_format(".$fieldKey[1].",'%Y%m') ='".$thn.$bln."')");	//blm filter bln-thn
		}
		if ($jenis=="mutasi"){
		$str=" SELECT  p.NIK, p.NAMA, concat(c.KOTA,' - ', d.NAMA_DIV, ' - ',j.NAMA_JAB) MUTASI_BARU,
						...
						WHERE m.nik=p.nik  and m.id_cab=c.id_cabang and m.id_div=d.id_div and m.id_jab=j.id_jab and c.id_cabang=".$cabang->ID_CABANG." and ".(sizeof($fieldKey)<=1?"date_format(".$fieldKey[0].",'%Y%m') ='".$thn.$bln."'":"(date_format(".$fieldKey[0].",'%Y%m') ='".$thn.$bln."' or date_format(".$fieldKey[1].",'%Y%m') ='".$thn.$bln."')")." and flag<>0"; //blm filter bln-thn
		}
	}
		//echo $str;
?>
		...
		foreach($result as $row){
			...
			switch($jenis){
				...
				case "resign":
					...
					echo "<td>".$row->APPROVED_RO_BY."</td>";
					echo "<td>".$row->APPROVED_PUSAT_BY."</td>";
					break;
				case "cuti":
					echo "<td>".$row->POSISI."</td>";
...
echo "<br>";
if ($display==0){
	  $param=$jenis."_".$cabang->ID_CABANG."_1_".$bln."_".$thn;
?>
