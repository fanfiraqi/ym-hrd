<?php errorHandler();?>
<?php echo form_open('prestasi/create',array('class'=>'form-horizontal','id'=>'myform'));?>
<div class="row">
	<div class="col-md-6">
...
<div class="row">
	<div class="col-md-6">
		<div class="form-group"><label  class="col-sm-4 control-label">PRESTASI</label>
			<div class="col-sm-8"><?=form_input(array('name'=>'prestasi','id'=>'prestasi','class'=>'form-control'));?>	</div>
		</div>
	</div>
</div>
...
$('#myform').submit(function(event) {
	$(this).saveForm('<?php echo base_url('prestasi/create');?>','<?php echo base_url('prestasi');?>');
...
$("#nama").autocomplete({
	...
	function(req, add){
		$.ajax({
			url: "<?php echo base_url('cuti/cariPeg'); ?>",
			dataType: 'json',
			type: 'POST',
			data: req,
			success:   
