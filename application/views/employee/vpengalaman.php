...
<div class="row">
	<div class="col-md-6">
		<div class="form-group"><label class="col-sm-4 control-label">PENGALAMAN KERJA</label>
...
<script>
    $(document).ready(function() {
        $('#dataTables-list').dataTable({
			...
			"sAjaxSource": "<?php echo base_url('employee_data/json_data_kerja');?>"
		});
    });
...
$("#nama").autocomplete({
	...
	function delThis(idx,  str){	//nik, nama
			var pilih=confirm('Apakah data Pengalaman Kerja  '+str+ ' akan dihapus ?');
			if (pilih==true) {
					$.ajax({
					type	: "POST",
					url		: "<?php echo base_url('employee_data/delThis');?>",
					data	: "idx="+idx+"&proses=peg_pengalaman_kerja"+"&field=id",
					timeout	: 3000,  
	...
	function editThis(obj){
		...
		$.ajax({
			...
			data: "id="+id+"&tabel=peg_pengalaman_kerja"+"&field=id",
			dataType: 'json',
			success: function(msg) {
	...
	$('#btsimpan').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: "<?php echo base_url('employee_data/saveData_kerja');?>",
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
