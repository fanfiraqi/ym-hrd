<div class="control-group"><a href="javascript:void(0)" id="btcreate" class="btn btn-primary">Entri Data</a></div>
...
<div class="row">
	<div class="col-md-6">
		<div class="form-group"><label class="col-sm-4 control-label">PENGALAMAN ORGANISASI / KEGIATAN SOSIAL</label>
...
<script>
    $(document).ready(function() {
        $('#dataTables-list').dataTable({
			...
			"sAjaxSource": "<?php echo base_url('employee_data/json_data_organisasi');?>"
		});
    });
...
$("#nama").autocomplete({
	...
	function delThis(idx,  str){	//nik, nama
			var pilih=confirm('Apakah data Pengalaman Organisasi  '+str+ ' akan dihapus ?');
			if (pilih==true) {
					$.ajax({
					type	: "POST",
					url		: "<?php echo base_url('employee_data/delThis');?>",
					data	: "idx="+idx+"&proses=peg_organisasi"+"&field=id",
					timeout	: 3000,  
	...
	function editThis(obj){
		...
		$.ajax({
			...
			data: "id="+id+"&tabel=peg_organisasi"+"&field=id",
			dataType: 'json',
			success: function(msg) {
	...
	$('#btsimpan').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: "<?php echo base_url('employee_data/saveData_organisasi');?>",
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
