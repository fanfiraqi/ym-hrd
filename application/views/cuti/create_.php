...
<div class="row">
	<div class="col-md-12">
		<div class="form-group">
			...
			<div id="ctthn_info" class="no-display">
			<label for="jatah" class="col-sm-2 control-label">Sisa Cuti Awal</label>
			<div class="col-sm-2">
				<?php
					$jatah = array(
						'name'=>'jatah',
			...
			<label for="sisacuti" class="col-sm-2 control-label">Sisa Cuti Akhir</label>
			<div class="col-sm-2">
				<?php
					$sisacuti = array(
						'name'=>'sisacuti',
...
<div class="row">
	<div class="col-md-6">
		<INPUT type="hidden" value="" name="checktable_cuti_saldo" id="checktable_cuti_saldo">
			<?php 
			$btsubmit = array(
					'name'=>'btsubmit',
					'id'=>'btsubmit',
			...
			$.ajax({
				...
				function(data){
					if(data.response =="true"){
						...
						if ($('#cuti').val()==1 ){
							if ($('#checktable_cuti_saldo').val()>0){
								$('#sisacuti').val(parseInt($('#jatah').val()) - (jmlhari));
							}else{
								$('#sisacuti').val(parseInt($('#jatah').val()));
							}
							
						}
						alert("sisa="+$('#sisacuti').val());
						if ($('#cuti').val()==2){
							$('#jatah').val(0);
							$('#sisacuti').val(0);
							/*if($('#subcuti').val()=='10'){
...
$("#nama").autocomplete({
	...
	function(event, ui) {                   
		...
		$("#checktable_cuti_saldo").text(ui.item.checktable_cuti_saldo);
		...
		if (ui.item.stsCutiOk=='FALSE')	{
			...
			$('#cuti').append('<option value="2" >IJIN</option>');
...
function setJenisCuti(){
	$('#cuti').find('option').remove().end();
	$('#cuti').append('<option value="2" selected>IJIN </option>');
	$('#cuti').append('<option value="3" >SAKIT</option>');
	$('#cuti').trigger('change');	
