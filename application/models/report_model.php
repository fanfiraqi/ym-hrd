...
class report_model extends MY_Model {
	...
	function view_mutasi($nik=null, $mutasi_baru=null){	
		
		$str = "SELECT  p.NAMA, '".$mutasi_baru."' MUTASI_BARU, ''  MUTASI_LAMA, m.*
	...
	function getArr_zisco_divAtCab_(){
		$str = "SELECT distinct mst.ID_CAB, mc.KOTA from mst_struktur mst left join mst_cabang mc on mst.id_cab=mc.id_cabang where mst.id_div =26
				ORDER BY mc.id_cabang ASC";
		return $this->gate_db->query($str)->result();		
	}
	function getArr_zisco_divAtCab($id_cab){
		$str = "SELECT distinct mst.ID_DIV, md.NAMA_DIV from mst_struktur mst inner join mst_divisi md on mst.id_div=md.id_div
				and mst.id_cab=$id_cab and mst.id_div =24
				ORDER BY mst.ID_DIV ASC";
