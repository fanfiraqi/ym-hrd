...
class ubudiah_fo extends MY_App {
	...
	public function index(){
		$this->template->set('pagetitle','Form Entri Penilaian Ubudiah FO Bulanan');		
		...
		$this->template->load('default','fgaji_fo/ubudiahFilter',$data);
	}
	...
	public function ubudiahEntri(){
		...
		$strCek="select count(*) CEK from ubudiah_fo where bln='".$bln."' and thn='$thn' and nik in (select distinct nik from pegawai where id_cabang=$id_cab  and status_aktif=1 and  id_jab =14)";
		$rsCek = $this->db->query($strCek)->row();
		
		$strList = "SELECT p.*, j.NAMA_JAB, period_diff( date_format( now( ) , '%Y%m' ) , date_format( tgl_aktif, '%Y%m' ) ) SELISIH from pegawai p, mst_jabatan j where p.id_jab=j.id_jab and status_aktif=1 and id_cabang=$id_cab and  p.id_jab=14 ";
		...
		if ($rsCek->CEK<=0 && $thn.$bln>=date('Ym')){
			$this->template->set('pagetitle','Form Entri Penilaian Ubudiah FO Bulanan (NEW) '.$blnStr[$bln]." ".$thn." Cabang ".strtoupper($nmCabang->KOTA));	
			$this->template->load('default','fgaji_fo/ubudiahEntri',$data);
		}else{
			...
			if (strtotime(date('Y-m-d'))>=$tgl1 && strtotime(date('Y-m-d'))<=$tgl2){	//cek sysdate antara 26-bln-1 s.d 25-bln 
				...
				$this->template->set('pagetitle','Form Entri Penilaian Ubudiah FO Bulanan (EDIT) '.$blnStr[$bln]." ".$thn." Cabang ".strtoupper($nmCabang->KOTA));	
				$this->template->load('default','fgaji_fo/ubudiahEntri',$data);
			}else{
				$strDR = "SELECT p.*, dr.JML_HARI,  j.NAMA_JAB, period_diff( date_format( now( ) , '%Y%m' ) , date_format( tgl_aktif, '%Y%m' ) ) SELISIH from pegawai p, mst_jabatan j, ubudiah_fo dr where p.nik=dr.nik and thn='$thn' and bln='$bln' and p.id_jab=j.id_jab and status_aktif=1 and id_cabang=$id_cab and p.id_jab =14 ";
				$data['row'] = $this->db->query($strDR)->result();
				$this->template->set('pagetitle','Form Entri Penilaian Ubudiah FO Bulanan (DISABLED) '.$blnStr[$bln]." ".$thn." Cabang ".strtoupper($nmCabang->KOTA));	
				$this->template->load('default','fgaji_fo/ubudiahEntri_disabled',$data);
			}
		}
	}
		...
		if ($this->input->is_ajax_request()){
			...
			if ($this->form_validation->run() == TRUE){
				try {
					$this->db->trans_begin();
					for($r=1;$r<=$this->input->post('jmlRow');$r++){
						$this->db->delete('ubudiah_fo', array("NIK"=>$this->input->post('nik_'.$r), 'BLN'=>$this->input->post('bln'), 'THN'=>$this->input->post('thn')));
						...
						if ($this->db->insert('ubudiah_fo', $data)){
							$this->db->trans_commit();
						} else {
