...
class pelanggaran extends MY_App {
	...
	{
		$this->config->set_item('mySubMenu', 'mn26');
		...
		$this->template->set('pagetitle','Daftar Catatan pelanggaran');		
		$this->template->load('default','employee/vpelanggaran',$data);
	}
	...
	public function json_data(){
		//if ($this->input->is_ajax_request()){
			...
			$str = "select pegawai.NIK,pegawai.NAMA, pegawai.ID_CABANG,pegawai.ID_DIV,pegawai.ID_JAB ,p.*  from pegawai, `pelanggaran` p  where  pegawai.nik=p.nik and pegawai.nik in (select distinct nik from `pelanggaran`)   ".($this->session->userdata('auth')->id_cabang>1?"  and id_cabang=".$this->session->userdata('auth')->id_cabang."  ":"  and id_cabang=".$cabang."  ");
			
			if ( $_GET['sSearch'] != "" )
			{
				
				$str .= " AND NAMA like '%".mysql_real_escape_string( $_GET['sSearch'] )."%' or nama_pelanggaran like '%".mysql_real_escape_string( $_GET['sSearch'] )."%'";
	...
	public function editThis(){
		...
		$str="select d.*, p.NAMA, p.id_cabang, p.id_div, p.id_jab from pelanggaran d, pegawai p where p.nik=d.nik and d.id='".$id."'";
			...
			if ($this->form_validation->run() == TRUE){
				...
				try {
					...
					if($state=="add"){ 
						...
						if ($this->db->where('ID',$state)->update('pelanggaran',$data)){
									$this->db->trans_commit();
									$respon->status = 'success';
						} else {
							throw new Exception("gagal simpan");
