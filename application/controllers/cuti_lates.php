		...
		//if ($this->input->is_ajax_request()){
			...
			foreach($query as $row){
				...
				$aaData[] = array(
					...
					'TGL_IJIN'=>revdate($row->TGL_AWAL).' s/d '.revdate($row->TGL_AKHIR),
					'KETERANGAN'=>$row->KETERANGAN,
					'ACTION'=>'<a href="javascript:void(0)" data-base="'.base_url().'" data-url="'.base_url('cuti/approve/'.$row->ID).'" data-id="'.$row->ID.'" data-sts="'.$status.'"  data-isactive="'.$row->ISACTIVE.'" onclick="detail(this)" class="btn btn-act btn-success"><i class="fa '.($status==1?" fa-eye ":" fa-gear ").'" title="Validasi"></i></a>'
				);
			}
				...
				try {
					...
					if ($tglawal=="00-00-0000" ||$tglakhir =="00-00-0000"){
						...
						if ($this->input->post('cuti')==1){
							...
							$jmlctthn=12-$this->input->post('jmlhari');
							if (sizeof($ccek)<=0){	//disini reset saldo cuti bila ganti tahun
								$this->db->query("insert into cuti_saldo (TAHUN, NIK, SISACUTI, UPDATED_DATE) values(".date('Y').", '".$nik."', 12, '".date ('Y-m-d H:i:s ')."') ");
								
							}
							//cuti_saldo stlh approve baru update sisa cuti
							/*else{
								$this->db->query("update cuti_saldo set SISACUTI = sisacuti-".$this->input->post('jmlhari')." where nik='".$nik."'");
							}*/
						}
...
WHERE p.tgl_aktif IS NOT NULL AND p.`STATUS_AKTIF`=1 AND p.`NAMA` LIKE '%{$keyword}%' OR p.`NIK` LIKE '%{$keyword}%') ss where ss.CUTIOK='TRUE'"*/
		...
		$str = "SELECT IF (DATE_ADD(p.tgl_aktif,INTERVAL 12 MONTH) <= DATE_FORMAT(SYSDATE(),'%Y-%m-%d'),'TRUE','FALSE') CUTIOK, IFNULL((SELECT sisacuti FROM cuti_saldo WHERE nik=p.`NIK` ORDER BY tahun DESC LIMIT 1),0 ) SISA_CUTI,  p.* FROM v_pegawai p  WHERE p.tgl_aktif IS NOT NULL AND p.`STATUS_AKTIF`=1  AND p.`NAMA` LIKE '%{$keyword}%' OR p.`NIK` LIKE '%{$keyword}%' ";
		...
		if( ! empty($query) )
		{
			...
			{	/*if($row->CUTIOK=='FALSE'){
				...
				$cuti_saldo_cek="select count(*) jml from cuti_saldo where tahun ='".date('Y')."' and nik ='".$row->NIK."'";
				$rs_cs=$this->db->query($cuti_saldo_cek)->row();
				...
				$data['message'][] = array(
					...
					'checktable_cuti_saldo' => $rs_cs->jml,
	...
	public function approve($id){
		...
		try {
			$data = array(
				'APPROVED' => $sts,
				'APPROVED_DATE' => ($sts==1?date('Y-m-d H:i:s'):'0000-00-00 00:00:00'),
			...
			if($rsCek->JENIS_CUTI==1){
				...
				if (sizeof($ccek)>=1){
					if (substr($rsCek->TGL_TRANS,0,4)==date('Y')){
						$this->db->query("update cuti_saldo set sisacuti=sisacuti-".$rsCek->JML_HARI." where NIK='".$rsCek->NIK."' and TAHUN=".substr($rsCek->TGL_TRANS,0,4)) ;
					}else {
