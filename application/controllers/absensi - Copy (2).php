...
class absensi extends MY_App {
	...
	public function doupload()
	{
		...
		for ($row = 2; $row <= $highestRow; ++$row) {
			...
			$data[]=array(
				...
				'LEMBUR' => $objWorksheet->getCellByColumnAndRow(10, $row)->getValue(),
				'LEMBUR_LIBUR' => $objWorksheet->getCellByColumnAndRow(11, $row)->getValue(),	
	...
	public function rekap_list(){
		...
		if ($rsCek->CEK<=0){
			...
			if ($data['thn'].$data['digitBln']==date('Ym') && $rsCek->VALIDASI !=1){				
				$data["status"]="OPEN";
				$this->template->set('pagetitle','Rekap Absensi Karyawan (OPEN) untuk Bulan '.$blnStr[$bln]." ".$thn." Cabang ".strtoupper($nmCabang->KOTA));		
				...
				$this->template->set('pagetitle','Rekap Absensi Karyawan (CLOSED/VIEW ONLY) '.$blnStr[$bln]." ".$thn." Cabang ".strtoupper($nmCabang->KOTA));	
	...
	public function resetTableAbsen(){
		...
		$respon = new StdClass();
		$respon->status = 'error';
		if ($this->db->query("delete from absensi where lokasi_id=$cabang")){
			$respon->status = 'success';
		}
		echo json_encode($respon);
	}
